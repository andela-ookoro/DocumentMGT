...
module.exports = {
  ...
  login: function login(req, res) {
    ...
    if (user.email && user.password) {
      ...
      return User.find({
        ...
        return (0, _utilities.sendMessage)(res, '' + (err.message || err), 401);
      });
    }
  ...
  */
      ...
      return User.find({
        ...
        if (foundUser) {
          ...
          switch (foundUser.status) {
            case disabled:
              (0, _utilities.sendMessage)(res, 'This account is blocked, Please account admin', 401);
              break;
            default:
  ...
  getUser: function getUser(req, res) {
    ...
    if (!req.user.isAdmin && userId !== req.user.id) {
      return (0, _utilities.sendMessage)(res, 'Unathorized operation, please check user id passed', 500);
    }
    var fetchUserQuery = '\n     select fname, lname, mname, "users"."roleId", "roles"."title" , email\n     from users \n     inner join roles\n     on "users"."roleId" = "roles".id\n     where "users"."id"= ' + userId;
    sequelize.query(fetchUserQuery).then(function (result) {
      var user = result[0][0];
      if (!user) {
        return (0, _utilities.sendMessage)(res, 'User was not found.', 404);
      }
      return (0, _utilities.sendData)(res, user, 200, 'user');
    }).catch(function (error) {
      return (0, _utilities.sendMessage)(res, error.message, 500);
    });
  },
  ...
  deleteUser: function deleteUser(req, res) {
    ...
    return User.findOne({
      ...
      return user.update({
        ...
        return (0, _utilities.sendData)(res, 'Your account has been deleted', 200, 'message');
      }).catch(function (error) {
   ...
   * @return {string } - user fullname
   */
  blockUser: function blockUser(req, res) {
    // convert param to int
    var userId = parseInt(req.params.id, 10);
    if (isNaN(userId)) {
      return (0, _utilities.sendMessage)(res, 'Invalid user ID', 400);
    ...
    return User.findOne({
      ...
      return user.update({ status: 'disabled' }).then(function () {
        return (0, _utilities.sendData)(res, 'User has been blocked successfully', 200, 'message');
      }).catch(function (error) {
        return (0, _utilities.sendMessage)(res, error.message, 400);
      });
    }).catch(function (error) {
  ...
  * @return {string } - user fullname
  */
  restoreUser: function restoreUser(req, res) {
    // convert param to int
    var userId = parseInt(req.params.id, 10);
    if (isNaN(userId)) {
      return (0, _utilities.sendMessage)(res, 'Invalid user ID', 400);
    ...
    return User.findOne({
      ...
      return user.update({ status: 'active' }).then(function () {
        return (0, _utilities.sendData)(res, 'User has been restored successfully', 200, 'messsage');
      }).catch(function (error) {
        return (0, _utilities.sendMessage)(res, error.message, 400);
      });
    }).catch(function (error) {
  ...
  updateUser: function updateUser(req, res) {
    ...
    return User.findOne({
      ...
      if (update.password) {
        ...
        if (!pass) {
          return (0, _utilities.sendMessage)(res, 'Unauthorized operation, check the credential provided', 403);
        }
      }
      return foundUser.update(_extends({}, update)).then(function (updateUser) {
  ...
  * @return {object} - users
  */
  lookupUser: function lookupUser(req, res) {
    // get new user info
    var query = req.query;
    // get user with this id
    return User.findAndCountAll({
      ...
      (0, _utilities.sendData)(res, users, 200, 'user');
    }).catch(function (error) {
      return (0, _utilities.sendMessage)(res, error.message, 400);
    });
  },
  ...
  */
  getUserDocument: function getUserDocument(req, res) {
    // get pagination
    var offset = parseInt(req.query.offset, 10) || 0;
